...
export default class AdvancementManager extends Application {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      classes: ["genefunk2090", "advancement", "flow"],
      template: "systems/genefunk2090/templates/advancement/advancement-manager.hbs",
      width: 460,
      height: "auto"
    });
  }
  ...
  get title() {
    ...
    const step = visibleIndex < 0 ? "" : game.i18n.format("GENEFUNK2090.AdvancementManagerSteps", {
      current: visibleIndex + 1,
      total: visibleSteps.length
    });
    return `${game.i18n.localize("GENEFUNK2090.AdvancementManagerTitle")} ${step}`;
  }
  ...
  getData() {
    ...
    return {
      ...
      header: item.name,
      subheader: level ? game.i18n.format("GENEFUNK2090.AdvancementLevelHeader", { level }) : "",
      steps: {
        current: visibleIndex + 1,
        total: visibleSteps.length,
  ...
  render(...args) {
     ...
     * @function genefunk2090.preAdvancementManagerRender
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager about to be rendered
     */
    const allowed = Hooks.call("genefunk2090.preAdvancementManagerRender", this);
  ...
  async close(options={}) {
    if ( !options.skipConfirmation ) {
      return new Dialog({
        title: `${game.i18n.localize("GENEFUNK2090.AdvancementManagerCloseTitle")}: ${this.actor.name}`,
        content: game.i18n.localize("GENEFUNK2090.AdvancementManagerCloseMessage"),
        buttons: {
          close: {
            icon: '<i class="fas fa-times"></i>',
            label: game.i18n.localize("GENEFUNK2090.AdvancementManagerCloseButtonStop"),
            callback: () => super.close(options)
          },
          continue: {
            icon: '<i class="fas fa-chevron-right"></i>',
            label: game.i18n.localize("GENEFUNK2090.AdvancementManagerCloseButtonContinue")
          }
        },
        default: "close"
      }).render(true);
  ...
  async _restart(event) {
    const restart = await Dialog.confirm({
      title: game.i18n.localize("GENEFUNK2090.AdvancementManagerRestartConfirmTitle"),
      content: game.i18n.localize("GENEFUNK2090.AdvancementManagerRestartConfirm")
    });
    if ( !restart ) return;
  ...
  async _complete(event) {
     ...
     * are applied.
     * @function genefunk2090.preAdvancementManagerComplete
     * @memberof hookEvents
     ...
     */
    if ( Hooks.call("genefunk2090.preAdvancementManagerComplete", this, updates, toCreate, toUpdate, toDelete) === false ) {
     ...
     * @function genefunk2090.advancementManagerComplete
     * @memberof hookEvents
     * @param {AdvancementManager} advancementManager The advancement manager that just completed
     */
    Hooks.callAll("genefunk2090.advancementManagerComplete", this);
