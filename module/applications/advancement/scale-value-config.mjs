...
export default class ScaleValueConfig extends AdvancementConfig {
  ...
  getData() {
    ...
    return foundry.utils.mergeObject(super.getData(), {
      ...
      movementUnits: CONFIG.GENEFUNK2090.movementUnits
    });
  }
  ...
  _prepareLevelData() {
    let lastValue = null;
    let levels = Array.fromRange(CONFIG.GENEFUNK2090.maxLevel + 1);
  ...
  _onIdentifierHintCopy(event) {
    ...
    game.tooltip.activate(event.target, {text: game.i18n.localize("GENEFUNK2090.IdentifierCopied"), direction: "UP"});
  }
  ...
  async _updateObject(event, formData) {
    ...
    if ( typeChange && (updates.configuration.type !== this.advancement.configuration.type) ) {
      ...
      await this.advancement.update(Array.fromRange(CONFIG.GENEFUNK2090.maxLevel, 1).reduce((obj, lvl) => {
