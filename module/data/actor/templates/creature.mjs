...
export default class CreatureTemplate extends CommonTemplate {
  static defineSchema() {
    return this.mergeSchema(super.defineSchema(), {
      bonuses: new foundry.data.fields.SchemaField({
        mwak: makeAttackBonuses({label: "GENEFUNK2090.BonusMWAttack"}),
        rwak: makeAttackBonuses({label: "GENEFUNK2090.BonusRWAttack"}),
        msak: makeAttackBonuses({label: "GENEFUNK2090.BonusMSAttack"}),
        rsak: makeAttackBonuses({label: "GENEFUNK2090.BonusRSAttack"}),
        abilities: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "GENEFUNK2090.BonusAbilityCheck"}),
          save: new FormulaField({required: true, label: "GENEFUNK2090.BonusAbilitySave"}),
          skill: new FormulaField({required: true, label: "GENEFUNK2090.BonusAbilitySkill"})
        }, {label: "GENEFUNK2090.BonusAbility"}),
        spell: new foundry.data.fields.SchemaField({
          dc: new FormulaField({required: true, deterministic: true, label: "GENEFUNK2090.BonusSpellDC"})
        }, {label: "GENEFUNK2090.BonusSpell"})
      }, {label: "GENEFUNK2090.Bonuses"}),
      skills: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, min: 0, max: 2, step: 0.5, initial: 0, label: "GENEFUNK2090.ProficiencyLevel"
        }),
        ability: new foundry.data.fields.StringField({required: true, initial: "dex", label: "GENEFUNK2090.Ability"}),
        bonuses: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "GENEFUNK2090.SkillBonusCheck"}),
          passive: new FormulaField({required: true, label: "GENEFUNK2090.SkillBonusPassive"})
        }, {label: "GENEFUNK2090.SkillBonuses"})
      }), {
        initialKeys: CONFIG.GENEFUNK2090.skills, initialValue: this._initialSkillValue,
        initialKeysOnly: true, label: "GENEFUNK2090.Skills"
      }),
      tools: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          required: true, nullable: false, min: 0, max: 2, step: 0.5, initial: 1, label: "GENEFUNK2090.ProficiencyLevel"
        }),
        ability: new foundry.data.fields.StringField({required: true, initial: "int", label: "GENEFUNK2090.Ability"}),
        bonuses: new foundry.data.fields.SchemaField({
          check: new FormulaField({required: true, label: "GENEFUNK2090.CheckBonus"})
        }, {label: "GENEFUNK2090.ToolBonuses"})
      })),
      spells: new MappingField(new foundry.data.fields.SchemaField({
        value: new foundry.data.fields.NumberField({
          nullable: false, integer: true, min: 0, initial: 0, label: "GENEFUNK2090.SpellProgAvailable"
        }),
        override: new foundry.data.fields.NumberField({
          integer: true, min: 0, label: "GENEFUNK2090.SpellProgOverride"
        })
      }), {initialKeys: this._spellLevels, label: "GENEFUNK2090.SpellLevels"})
    });
  }
  ...
  static _initialSkillValue(key, initial) {
    if ( CONFIG.GENEFUNK2090.skills[key]?.ability ) initial.ability = CONFIG.GENEFUNK2090.skills[key].ability;
    return initial;
  }
  ...
  static get _spellLevels() {
    const levels = Object.keys(CONFIG.GENEFUNK2090.spellLevels).filter(a => a !== "0").map(l => `spell${l}`);
    return [...levels, "pact"];
  }
  ...
  static #migrateSensesData(source) {
    ...
    for ( let s of original.split(",") ) {
      ...
      if ( (type in CONFIG.GENEFUNK2090.senses) && !(type in source.attributes.senses) ) {
  ...
  static #migrateToolData(source) {
    ...
    for ( const prof of original.value ) {
      const validProf = (prof in CONFIG.GENEFUNK2090.toolProficiencies) || (prof in CONFIG.GENEFUNK2090.toolIds);
...
function makeAttackBonuses(schemaOptions={}) {
  return new foundry.data.fields.SchemaField({
    attack: new FormulaField({required: true, label: "GENEFUNK2090.BonusAttack"}),
    damage: new FormulaField({required: true, label: "GENEFUNK2090.BonusDamage"})
  }, schemaOptions);
}
